...
const traverseRoom = (node, callback) => {
    traverseNode(node, callback);
    node.path.get().forEach((pathId) => {
        const path = getRoomSegment(node.core, pathId);
        traverseNode(path, callback);
    });
    node.holes.get().forEach((hole) => {
        hole.forEach((holeId) => {
            const hole = getRoomSegment(node.core, holeId);
            traverseNode(hole, callback);
        });
    });
};
