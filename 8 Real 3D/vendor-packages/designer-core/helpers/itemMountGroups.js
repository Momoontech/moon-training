 ...
 */
const isCeilingMountedNode = (core, nodeId) => {
    const node = getOptionalNode(core, nodeId);
    if (!node)
        return false;
    const mount = getOptionalNode(core, node.parent.get());
    if (mount && mount.type === NodeType.MountPlane && mount.mountSlotTypes.get().includes(MountType.ceiling)) {
        return true;
    }
    if (node.type !== NodeType.Item)
        return false;
    const mountTypes = node.mountTypes.get();
    return (mountTypes.includes(MountType.ceiling) &&
        !mountTypes.includes(MountType.floor) &&
        !mountTypes.includes(MountType.wall));
};
/**
 ...
 */
const shareMountGroup = (core, nodeId, otherNodeId) => isCeilingMountedNode(core, nodeId) === isCeilingMountedNode(core, otherNodeId);

export { isCeilingMountedNode, shareMountGroup };
